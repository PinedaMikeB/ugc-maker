...
function buildFoodScript(products, offer) {
  ...
  return [
    `Hook: If you are craving something soft, sweet, and bakery fresh, start with ${productLead}${primaryPrice ? ` at ${primaryPrice}` : ""}.`,
    ...
    `Body 2: Follow with ${secondaryLead}${third ? ` and ${thirdLead}` : ""} so the lineup feels bigger than one item.`,
...
export function prepareGeneratorInput({
  source,
  productDetails,
  audioMode,
  voiceScript,
