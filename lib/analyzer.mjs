...
function scoreProduct(product, offerText, offerKeywords, referenceKeywords) {
...
function pickRelevantProducts({ offer, reference, website }) {
  ...
  const ranked = products
    .map((product) => ({
      ...product,
      _score: scoreProduct(product, offerText, offerKeywords, referenceKeywords),
    }))
...
function buildFoodScript(products, offer) {
  const primary = products[0];
  const secondary = products[1];
  const third = products[2];
  const productLead = primary?.name || "your hero product";
  ...
  const painPoint = inferPainPoint(products, offer);

  return [
    `Hook: You know ${painPoint}? Start with ${productLead}${primaryPrice ? ` at ${primaryPrice}` : ""}.`,
    ...
    `Body 2: Make it feel real by following with ${secondaryLead}${third ? ` and ${thirdLead}` : ""}, like a friend showing what else looked good at the counter.`,
    `CTA: Close with a soft invite to try it fresh today ${brandPhrase}.`,
  ].join("\n");
}

function buildFoodAnalysisFallback({ offer, website, reference }) {
  const websiteLines = inferWebsiteProductLines(website);
  const relevantProducts = pickRelevantProducts({ offer, reference, website });
  ...
  const plan = [
    `1. Open on ${relevantProducts[0]?.name || "the most desirable product"} in the first two seconds.`,
  ...
  const script = buildFoodScript(relevantProducts, trim(offer) || "your bakery line");

  const scenes = [
    `Scene 1: ${relevantProducts[0]?.name || "strongest hero product"} in the first 2 seconds.`,
    `Scene 2: extreme close-up of ${relevantProducts[0]?.name || "the hero item"} texture, filling, drizzle, or crumb.`,
  ...
  return { bestAdaptation, plan, script, scenes, relevantProducts };
}
...
async function maybeGenerateFoodAnalysisWithAI({ offer, website, reference, fallback }) {
  ...
  const prompt = [
    "Create a believable UGC-style bakery promo adaptation from a winning food reference.",
    "Return strict JSON with keys: bestAdaptation, plan, script, scenes.",
    "The script must sound like a real person, not ad copy.",
    "Use emotional connection, relatable pain points, and everyday situations.",
    "Avoid fake hype, avoid corporate wording, avoid exaggerated marketing language.",
    "Use the actual product names provided. Mention prices only if natural.",
    "Write in warm, natural Filipino-English friendly tone, but keep it concise.",
    "Make the script no more than 120 words.",
    "",
...
export async function analyzeInput({ referenceUrl, offer, websiteUrl }) {
  ...
  if (domain === "food") {
    const fallback = buildFoodAnalysisFallback({ offer: cleanOffer, website, reference });
    ...
    generated = aiGenerated ? { ...fallback, ...aiGenerated } : fallback;
    if (aiGenerated) {
      scriptEngine = "openai";
    }
  } else {
...
export async function prepareGeneratorInput({
  ...
  const fallbackScript = buildFoodScript(relevantProducts, cleanDetails || "your bakery line");
