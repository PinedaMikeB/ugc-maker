...
function buildFoodScript(products, offer, heroProduct, customerPain) {
  ...
  return [
    `Hook: Bakit ngayon ko lang nadiscover 'to? You know ${painPoint}? ${productLead}${primaryPrice ? ` at ${primaryPrice}` : ""} feels like the kind of merienda you actually look forward to.`,
    ...
    `CTA: Frame it like a hidden gem people should not miss, then close with a soft invite to try it fresh today ${brandPhrase}.`,
  ].join("\n");
}
...
function buildFoodAnalysisFallback({ offer, heroProduct, customerPain, website, reference }) {
  ...
  const bestAdaptation = [
    ...
    `Campaign angle: make people feel like they just discovered a hidden gem bakery or bread they will keep coming back for.`,
...
async function maybeGenerateFoodAnalysisWithAI({
  ...
  const prompt = [
    ...
    "The campaign angle should make people feel like they discovered a hidden gem bakery or a bread they cannot believe they only found now.",
    "Use hidden-gem discovery framing naturally in the hook and recommendation.",
    "Lean into lines like: 'Bakit ngayon ko lang nadiscover tong bakery na to?', 'Ito yung tinapay na babalikan mo.', 'Hidden bakery na hindi mo dapat palampasin.', 'Softest bread I found this week.'",
